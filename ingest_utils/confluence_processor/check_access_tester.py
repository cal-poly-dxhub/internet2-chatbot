...
SERVICE_ACCOUNT_FILE = '/Users/kartik/AWS/Internet-II/internet-ii-ingest-1/chatbot-i2-drive-reader.json'
...
service_account_service = build('drive', 'v3', credentials=creds,developerKey=api_key)

# Original folder to test
original_folder_id = '13CiH9pKBxAI5zl-LrgjY-ZiV_zC5FxmX'
original_folder_url = f"https://drive.google.com/drive/folders/{original_folder_id}"
...
for folder_name, folder_id in folder_ids.items():
    try:
        ...
        folder = service_account_service.files().get(
            fileId=folder_id, 
            fields='id, name, permissions, owners, shared, capabilities'
        ).execute()

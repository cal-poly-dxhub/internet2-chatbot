...
class ConfluenceScraper:
    ...
    def scrape_assets(self) -> List[Dict]:
        ...
        target_table = None
        for table in tables:
            if table.find("th", string="Assets") or table.find(
                "th", string="Meeting Assets"
            ):
                target_table = table
                break
        ...
        for row in target_table.find("tbody").find_all("tr"):
            ...
            if (
                len(cols) < 5
            ):  # Ensure it's a data row not a header or empty row (bug befre)
                continue
            ...
            for tag in asset_tags:
                ...
                is_subscriber_content = (
                    "(subscribers)" in assets_col.get_text(strip=True).lower()
                )
                ...
                if file_type == "mp4":
                    ...
                    if (
                        link_url
                        and not link_url.endswith("/")
                        and "download/thumbnails" not in link_url
                    ):  # Exclude folder links, empty URLs, and Confluence thumbnails
                        current_block_assets.append(
                            {
                                "url": link_url,
                                "file_type": file_type,

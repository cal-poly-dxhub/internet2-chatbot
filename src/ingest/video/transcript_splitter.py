...
def load_scenes_from_csv(csv_file: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        header_line = next(line for line in data_lines if "Scene Number" in line)
...
def match_scenes_with_transcription(
    ...
    for item in tqdm(transcription_items, desc="Matching scenes with transcription"):
        if item["type"] == "pronunciation":
            ...
            while (
                ...
                if current_text:
                    ...
                    current_text = []
                    current_speaker = None  # Reset the speaker when changing scenes
...
def consolidate_short_scenes(
    ...
    consolidated = []
    current_chunk = {
        "number": matched_scenes[0]["number"],  # Keep only the first scene number
...
def process_video_scenes_and_transcription(
    ...
    try:
        ...
        matched_scenes = match_scenes_with_transcription(scenes, transcription_items)

        logger.info(
            f"Consolidating scenes to ensure minimum chunk length of {min_chunk_length} characters"
        )
        consolidated_chunks = consolidate_short_scenes(matched_scenes, min_chunk_length)

        logger.info(f"Writing output to {output_file}")
        output_data = []

        for chunk in tqdm(consolidated_chunks, desc="Processing consolidated chunks"):
            scene_file = next(
                (
                    file
...
if __name__ == "__main__":
    ...
    transcription_file = "./video/one_shot/transcriptions/video/transcription.json"

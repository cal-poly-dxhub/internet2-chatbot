...
def filter_platform_documents(
    ...
    response = invoke_model(
        filter_prompt,
        os.getenv("DOCUMENT_FILTER_MODEL_ID"),
        max_tokens=1000,
    )
...
def add_meeting_list(
    ...
    Args:
        text (str): Input text containing UUID references in format <uuid>
        metadata_mapping (Dict[str, Dict[str, Any]]): Mapping of UUIDs to metadata
            Format: {"uuid": {"parent_folder_name": str,
...
def extract_sources_and_meetings(
    text: str,
    uuid_mapping: Dict[str, Dict[str, Any]],
    metadata_mapping: Dict[str, Dict[str, Any]],
) -> tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """Extract sources and meetings from the processed text for JSON response."""
    sources = []
    meetings = []

    # Extract UUIDs that appear in the text
    uuid_pattern = r"([a-f0-9]{8})"
    referenced_uuids = set(re.findall(uuid_pattern, text))
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    try:
        ...
        # Generate embedding and fetch documents
        ...
        # Generate source + metadata mapping
        ...
        # Create simplified mapping for LLM prompt
        ...
        prompt: str = (
            ...
            + get_prompt("/chatbot/prompts/chat").format(
                ...
                citations=str(simplified_mapping),
            )
        )
        ...
        # Extract sources and meetings for JSON response
        sources, meetings = extract_sources_and_meetings(
            final_response, source_mapping, metadata_mapping
        )

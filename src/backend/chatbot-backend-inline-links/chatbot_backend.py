...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    try:
        ...
        # Reconstruct the response with inline links (like original markdown)
        clean_response = meeting_response
        # Remove UUID references
        clean_response = re.sub(r"<[a-f0-9]{8}>", "", clean_response)
        
        # Add inline source links back to the text
        for source in sources:
            # Find where this source should appear in the text and add the link
            source_title = source["title"]
            if source_title in clean_response:
                # Replace the title with a markdown link
                link_text = f"[{source_title}]({source['url']}) — _{source['badge']}_"
                clean_response = clean_response.replace(source_title, link_text)
        
        # Add meetings section at the end
        if meetings:
            meetings_text = "\n\n**Meetings referenced:**\n"
            for meeting in meetings:
                meetings_text += f"- [{meeting['name']}]({meeting['url']}) — {meeting['badge']}\n"
            clean_response += meetings_text
